...
gulp.task('tone', function () {
  return gulp.src(path.join(conf.paths.src, '/assets/tono/*'))
    .pipe(gulp.dest(path.join(conf.paths.dist, '/assets/tono')));
});

gulp.task('clean', function () {
  return $.del.sync([path.join(conf.paths.dist, '/**/*'), path.join(conf.paths.tmp, '/')]);
});

gulp.task('build', ['clean', 'html', 'env-config', 'tone']);
